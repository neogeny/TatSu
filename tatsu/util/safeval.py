    ...
    def make_pairs_hashable(items: Iterable[tuple[str, Any]]) -> Iterable[tuple[str, Any]]:
        for name, obj in items:
            if isinstance(obj, dict):
                yield name, tuple(make_pairs_hashable(obj.items()))
            elif isinstance(obj, (list, set, tuple)):
                yield name, tuple(make_pairs_hashable((str(i), elem) for i, elem in enumerate(obj)))
...
def _check_eval_safe_cached(expression: str, context_items: tuple[tuple[str, Any], ...]) -> None:
    ...
    for node in ast.walk(tree):
        # Forbidden nodes for "doesn't raise" and general safety
        if isinstance(node, (ast.Raise, ast.Try, ast.ExceptHandler)):
            raise SecurityError(f"Exception handling/raising is forbidden: {type(node).__name__}")

        # 1. Attribute Access Guard
...
def scan_for_exceptions(obj: Any, path: str = "context") -> None:
    """
    ...
    if isinstance(obj, Mapping):
        for k, v in obj.items():
            scan_for_exceptions(v, f"{path}['{k}']")
    elif isinstance(obj, Iterable) and not isinstance(obj, (str, bytes)):
        for i, item in enumerate(obj):
            scan_for_exceptions(item, f"{path}[{i}]")
...
def check_eval_context(context: dict[str, Any]) -> None:
    ...
    # New: Recursive scan for boxed Exceptions
    scan_for_exceptions(context)
