...
{{TatSu}} has many configuration options. They are all defined in `tatsu.parserconfig.ParserConfig`. With the introduction of `ParserConfig` there's no need to declare every configuration parameter as an optional named argument in entry points and internal methods. The defaults set in `ParserConfig` are suitable for most cases, and they are easy to override.
...
Entry points and internal methods in {{TatSu}} have an optional `config: ParserConfig | None = None` argument.
...
The class that implements tokenization for the parser. If it's not defined then the parsing modules will default to `buffering.Buffer`.

This option was applied in the prototype PEG parser for Python as to use the native Python tokenizer.
...
Enable or disable memoization in the parser. Only very specific input languages require this to be `False`.
...
Enables or disables memoization for lookaheads. Only very specific input languages require this to be `False`.
...
The size of the cache for memos. As parsing progresses, previous memos are rarely needed, so there's a bound to the number of memos saved (currently 1024).
...
Produce a trace of the parsing process. See the [Traces](traces.md) section for more information.
...
An alias for the [name](#name) option.
...
The list of keywords in the input language. See [Reserved Words and Keywords](syntax.md#reserved-words-and-keywords) for more information.
...
When set to `True`, avoids matching tokens when the next character in the input sequence is alphanumeric or a `@@namechar`. Defaults to `False`. See [token expression](syntax.md#text-or-text) for an explanation.
...
When `parseinfo==True`, a `parseinfo` entry is added to [AST](http://en.wikipedia.
org/wiki/Abstract_syntax_tree) nodes that are dict-like. The entry provides information about 
what was parsed and where. See [Abstract Syntax Trees](ast.md) for more information.
