...
You can also use [Python](http://python.org)'s built-in types as node types, and `ModelBuilderSemantics` will do the right thing:
...
Which nodes get *walked* is up to the `NodeWalker` implementation. Some strategies for walking *all* or *most* nodes are implemented as classes in `tatsu.walkers`, such as `PreOrderWalker` and `DepthFirstWalker`.
