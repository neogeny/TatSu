...
The file ``etc/tatsu.ebnf`` contains a grammar for the |TatSu| **TatSu** grammar
language written in its own grammar language. It is used in the
*bootstrap* test suite to prove that |TatSu| **TatSu** can generate a parser to
parse its own language, and the resulting parser is made the bootstrap
parser every time |TatSu| **TatSu** is stable (see ``tatsu/bootstrap.py`` for
the generated parser).

|TatSu| **TatSu** uses |TatSu| **TatSu** to translate grammars into parsers, so it is a
...
end-to-end example of how to implement a translator using |TatSu| **TatSu**.
...
provided by |TatSu| **TatSu**.

g2e
~~~

The project ``examples/g2e`` contains a `ANTLR`_ to |TatSu| **TatSu** grammar
...
|TatSu| **TatSu** grammar on standard output, but because the model used is
|TatSu| **TatSu**'s own, the same code can be used to directly generate a parser
